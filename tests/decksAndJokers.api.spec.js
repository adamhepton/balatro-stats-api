...
test.group('Decks and Jokers API', () => {
  ...
  test('fully ignores default parameters if all parameters passed', ({ assert }) => {
    assert.deepEqual(
      decksAndJokers(p, amendedParameters),
      decksAndJokers(p, {...expectedDefaults, ...amendedParameters})
    )
  });
  ...
  test('partially applies default parameters if type is missing', ({ assert }) => {
    ...
    assert.deepEqual(
      decksAndJokers(p, partialParameters),
      decksAndJokers(p, {...expectedDefaults, ...partialParameters})
    )
  });
